...
$(document).ready(function(){
	...
	$('#videoPageURI').change(function(){
		...
		else if(/^https?:/.test(vurldecode(videoPageURI))){
			$('#videoPageURI').val(vurldecode(videoPageURI));
			$('#videoPageURI').parent().removeClass('control-group error');
			$('#fetchButton').attr('disabled',false);
		}
		else{
	...
	if(videoPageURI==null){
		...
		if(videoPageURI.indexOf('http')===0){
			videoPageURI = vurldecode(decodeURIComponent(videoPageURI));
		} else {
			videoPageURI = vurldecode(videoPageURI);
...
window.getFlv = function() {
  ...
  //if($('#ggad').css('height')=='0px') scriptUrl = 'http://www.flvxz.com/js/adblocked.js';
	...
	//$.getScript("http://log.flvxz.com/?url="+vurlencode(uri));
}
...
window.v56pass = function(this_btn,vid,sid){
	...
	if(pass.length == 0) return;
	$(this_btn).val('正在验证密码').attr('disabled', 'disabled');
	$.getScript(baseurl + "/getFlv.php?url=" + vurlencode('http://www.56.com/fakeurl/vid/' + vid + "/sid/" + sid + "/password/" + pass));
}
...
function VideoUtil(){
	this.getInfo = function(url, callback){
		...
		var code =url.substring( url.lastIndexOf("/"), url.lastIndexOf("."));
		alert(code);
		$.get("http://api.tudou.com/v6/video/info?app_key=8988764315e38435&format=json&itemCodes="+code, function(data) {
			callback(data);
		});
	};
}
...
function generateUrl(session, decoded){
	//var session_url = window.location + "?session_id="+vurlencode(session.id);
	
	//TODO
}

function enable_generate(session_url){
	//TODO
}

function parseRealUrl(video_url, callback){
	var scriptUrl = "http://www.flvxz.com/getFlv.php?url=" + vurlencode(video_url);
	$.getScript(scriptUrl, function(){
		var video_url=$("#result").find("a").get(6).href;
		callback(video_url);
	});
}

function getVideoInfo(video_itemCode, callback) {
